...
- feat(mutants): mutation scope is the class of decision, not the file list (`324c3b1`)
- feat(hooks): a PostToolUse lint fast path and a SessionStart digest (`5a47a14`)
- feat: the Claude lifecycle hook is a fast path over the git hook (ADR-0027) (`4bf5006`)
- fix(lint): the vendored pre-commit hook cited decisions bare (`fb03d28`)
- fix(lint): a backticked citation is a mention of the form, not a use of it (`fb8f914`)
- docs(readme): the mutation list is scoped by decision, not by purity (`366571c`)
- docs(ledger): the wiring rule goes inert in a repo with no manifest (`2f20bab`)
- docs(claude): /wrap-up asks four questions, not three (`b1fad59`)
- docs(ledger): admit evidence that cannot be re-derived, not a record of work done (`2fe76d1`)
- chore: 0.4.2 — vendored hook citations; installed repos need /init-method --update (`2130671`)
- chore: untrack the tarball the ignore rule was added for (`a87158e`)
- chore: changelog for v0.4.1 (`6a3c400`)
- ci: run the mutation check instead of leaving it to be remembered (`501ebc4`)
